	...
	return []capability{
		{
			...
			Detail: []string{
				...
				"        them. Use `homelab ios` instead: a real iPhone on a real cable,",
				"        driven from here (`ios shot out.png --url <url>`).",
				"        Playwright WebKit still does not count, measured 2026-09-01: against",
				"        the 15 iOS defects in the session corpus it reproduces 2, both of",
				"        which headless Chromium already reproduces. Safe-area insets read",
				"        0px, maxTouchPoints is 0, standalone/display-mode cannot be faked,",
				"        and share/push are undefined. Say which platform you are",
				"        reproducing ON; if the report does not say, ask in one line.",
			},
		},
		{
			Intent:  "run, test or sideload something on a real iPhone",
			Use:     "homelab ios — a personal iPhone cabled to the London Mac, driven from here",
			Instead: "assuming there is no iOS instrument, or substituting the Android emulator",
			Synonyms: []string{"ios", "iphone", "ipad", "safari", "webkit", "apple",
				"sideload", "xcode", "swift", "testflight", "app store", "ipa",
				"native app", "mobile app", "on my phone", "real device", "appium",
				"webdriveragent", "install the app", "build for ios"},
			Detail: []string{
				"doctor: homelab ios doctor        every link, and what to do about each",
				"drive:  homelab ios shot out.png --url <url> | --tap X,Y",
				"build:  homelab ios install <xcode-project-dir>   free Apple ID, no paid",
				"        Developer Program membership needed",
				"limits: certificates last 7 DAYS, 3 apps installed at once (WebDriverAgent",
				"        holds one), 10 new App IDs per week. Reusing a bundle id is free.",
				"        A free team cannot sign push, App Groups, Live Activities, Wallet,",
				"        HealthKit or iCloud — those builds run with the feature missing.",
				"needs:  the phone UNLOCKED (Auto-Lock Never) and the Mac awake and logged",
				"        in. A locked phone can be screenshotted and nothing else, and a Mac",
				"        reboot needs a human because FileVault holds the disk.",
			},
		},
		{
			...
			Detail: []string{
				...
				"iOS/Safari: `homelab ios shot out.png --url <url>` drives real Safari on a",
				"        real iPhone. Every iOS browser uses the system WebKit, the Android",
				"        emulator cannot reproduce it, and Playwright WebKit on Linux is not",
				"        Safari — so this is the only instrument, and it is a real one.",
				"        Note the phone's DEFAULT browser is Chrome, so an unqualified deep",
				"        link opens Chrome; `ios shot --url` names Safari explicitly.",
			},
		},
		{
		...
		{
			Intent:  "find out whether we are actually using our own tooling, or drifting back to built-ins",
			Use:     "homelab reflect  (run|diff|history)",
			Instead: "guessing from memory whether a rule or a verb is working",
			Synonyms: []string{"reflect", "session study", "are we using", "drift", "tool blindness",
				"did the fix work", "trend", "improve tooling", "metrics on ourselves",
				"how often do we", "measure ourselves", "retrospective"},
			Detail: []string{
				"Deterministic detectors over the whole session corpus: seconds, no model",
				"        tokens. Records a snapshot to history/ (committed), pushes",
				"        session_reflection_* to Prometheus, prints the delta vs the last run.",
				"The point is the TREND — one run says we hand-roll 1,789 sleeps, two say",
				"        whether the reword worked.",
				"Found a miss by hand that no detector caught? Add it to patterns.json in",
				"        the same session — D2 under-counts by construction.",
				"The 36-agent judging pass is separate and on demand: workflow.js in",
				"        ~/code/docs/agents/session-reflection (6.8M tokens, ~105 min).",
			},
		},
		{
