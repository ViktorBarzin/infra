...
resource "helm_release" "traefik" {
  ...
  values = [yamlencode({
    ...
    experimental = {
      ...
      # "Plugins are disabled because an error has occurred.") including every
      # other entry in this map — after any change here verify plugin load in
      # the logs AND that paperless-mcp still gates. Since 2026-09-21 there is
      # no remote plugin left, so that failure can no longer be caused by
      # something outside this repo; see api-token-middleware below.
      ...
      localPlugins = {
        ...
        # plugins.traefik.io on every pod start, and Traefik disables EVERY
        # plugin when any one of them fails to load. A fetch timeout therefore
        # took the local crowdsec, realip and sablier plugins down with it,
        # which made traefik-crowdsec@kubernetescrd -- a websecure ENTRYPOINT
        # middleware, so every router references it -- unresolvable, which
        # invalidated every router on the entrypoint, which left that pod with
        # an empty routing table answering a bare 404 for every host on the
        # estate. Twice in three days: 2026-09-19T01:17:44Z and
        # 2026-09-21T02:30:24Z, the second for ~4h and ~2,700 requests, one
        # replica in three each time, while /ping kept the pod Ready and in
        # the Service endpoints.
        #
        # The download-plugins initContainer that used to guard this is GONE
        # because it never worked. It staged the archive under
        # /plugins-storage/archives/ and exited 0 having verified 1,424,602
        # bytes, and Traefik then cleaned that directory and fetched from
        # plugins.traefik.io anyway. Measured on the failed pod: its archives/
        # was empty, while a healthy pod's state.json was pretty-printed by
        # Traefik rather than written by the initContainer's compact printf.
        # Pre-seeding that path does not work. Vendoring does, which is what
        # the three plugins below have always done.
        #
        # Source pinned in-repo at ./api-token-plugin/ from upstream tag
        # v0.1.4, copied byte-for-byte out of a running pod's unpacked plugin
        # source (plugin.go 15673 B, go.mod 67 B, .traefik.yml 654 B; the
        # 1.4 MB release zip is images and docs). Stdlib only, no require
        # block. Upgrading = deliberately re-vendoring these files and
        # re-running scripts/yaegi-plugin-gate.
        api-token-middleware = {
          moduleName = "github.com/Aetherinox/traefik-api-token-middleware"
          mountPath  = "/plugins-local/src/github.com/Aetherinox/traefik-api-token-middleware"
          type       = "inlinePlugin"
          source = {
            "go.mod"       = file("${path.module}/api-token-plugin/go.mod")
            ".traefik.yml" = file("${path.module}/api-token-plugin/.traefik.yml")
            "plugin.go"    = file("${path.module}/api-token-plugin/plugin.go")
          }
        }
      }
    }
