...
1. **Anubis PoW challenge** (per-site reverse proxy) — `modules/kubernetes/anubis_instance/`. Latest: `ghcr.io/techarohq/anubis:v1.25.0`. Difficulty 2 (~250 ms desktop / ~700 ms mobile), 30-day JWT cookie scoped to `viktorbarzin.me` so a single solve covers every Anubis-fronted subdomain. Active on 7 stacks — authoritative list in `.claude/CLAUDE.md` (Anubis bullet). Add to a stack: `module "anubis" { source = "../../modules/kubernetes/anubis_instance"; name = "X"; namespace = ...; target_url = "http://<svc>.<ns>.svc.cluster.local" }`, then point ingress_factory at `module.anubis.service_name` + `port = module.anubis.service_port` and set `anti_ai_scraping = false`. **No X-Real-Ip wiring needed** — `ingress_factory` auto-attaches `traefik-real-ip@kubernetescrd` to any `anubis-*` backend, which superseded the old mandatory `strip_x_real_ip = true` flag when that machinery was retired 2026-07-19 (post-mortem 2026-07-14 covers why the header matters: Anubis binds its JWT to X-Real-Ip). Shared ed25519 signing key in Vault `secret/viktor` -> `anubis_ed25519_key`. **Custom policies pass `policy_rules_yaml`** — the rules list only, without the `bots:` key, which the module owns (renamed from `policy_yaml` 2026-08-22). **Local networks skip the challenge entirely**: the module renders a `trusted-local-networks` ALLOW rule first, matched on the real-ip-stamped X-Real-Ip, so LAN/VLAN/WireGuard/tailnet/in-cluster clients — humans and non-JS automation alike — reach the app directly. Canonical CIDR list is `var.trusted_local_cidrs`, mirrored as the x402 gateway's `TRUSTED_CIDRS` in `stacks/traefik`; change both. Design: `docs/plans/2026-08-22-local-network-bot-wall-bypass-design.md`. **Avoid putting Anubis in front of CLI/API/Git endpoints (Forgejo, APIs, WebDAV)** — clients without JS can't solve PoW.
