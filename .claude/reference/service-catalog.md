...
| orchestrator-sandbox | **TEMPORARY (2026-09-24)**, for the orchestrator bake-off (plan `~/code/docs/agents/2026-09-24-orchestrator-bake-off.md`, in the monorepo root rather than this repo). Namespace `orchestrator-sandbox` where Scion runs its agent pods: Pod Security Admission `baseline` (no privileged pods, host namespaces or hostPath), a 24Gi memory / 12 CPU quota on requests and limits with a matching LimitRange, and a `scion-broker` ServiceAccount whose namespace-only Role covers pods, exec/attach/log, secrets, configmaps and PVCs. Scion's hub on the devvm uses its long-lived token (`scion-broker-token`). Agent pods read `CLAUDE_CODE_OAUTH_TOKEN` from the `claude-oauth` Secret, an ExternalSecret of Vault `secret/orchestrator-sandbox` (wizard's own token, not shared with the fixer or claude-agent-service). Removed after the sprint unless Scion wins; teardown steps are at the top of `stacks/orchestrator-sandbox/main.tf`. | orchestrator-sandbox |
