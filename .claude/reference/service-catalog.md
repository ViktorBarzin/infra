...
| learn | learn Viewer — per-user web surface for /teach learning workspaces at `learn.viktorbarzin.me` (Authentik `auth=required`). K8s side only: Service+Endpoints → DevVM Caddy `10.0.10.10:7685`, which strips the @domain from `X-Authentik-Username` (a full email) and maps the local part to the OS user like ttyd/t3-dispatch (vbarzin→wizard etc., sync with `/etc/ttyd-user-map`) → `/home/<os_user>/code/learn` (live from disk, `file_server browse`; canonical Caddyfile `scripts/devvm-caddyfile`). Same header-trust model as ttyd/t3-dispatch. Decision: monorepo `learn/docs/adr/0001`. | learn |
