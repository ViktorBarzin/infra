...
| learn | learn Viewer — per-user web surface for /teach learning workspaces at `learn.viktorbarzin.me` (Authentik `auth=required`). K8s side only: Service+Endpoints → DevVM Caddy `10.0.10.10:7685`, which maps `X-Authentik-Username` → `/home/<user>/code/learn` (live from disk, `file_server browse`; canonical Caddyfile `scripts/devvm-caddyfile`). Same header-trust model as ttyd/t3-dispatch. Decision: monorepo `learn/docs/adr/0001`. | learn |
