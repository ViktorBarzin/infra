...
| reverse-proxy | Generic reverse proxy for devices outside K8s (router, iDRAC, NAS, Proxmox, HA). Small-buffer devices go through `lean-proxy` (openresty, 2 replicas), which sends them only an allowlisted header set and their own cookies; gw is on it since the 413 recurrence of 2026-09-22. See `docs/architecture/networking.md` → "Small-buffer devices behind the reverse proxy". | reverse-proxy |
...
| tripit | Self-hosted TripIt-clone travel-itinerary PWA (FastAPI + SvelteKit SPA, same-origin). CNPG (`tripit` db, Vault static role `pg-tripit`) + RWX NFS trip-doc vault (`/srv/nfs/tripit-documents`) + RWO `proxmox-lvm-encrypted` personal-document vault `tripit-personal-documents` (passports/IDs — AES-256-GCM app-layer envelope, master key `DOCUMENT_ENCRYPTION_KEY` in `secret/tripit`). `auth=required` (Authentik forward-auth, reads `X-authentik-email`); second `auth=none` ingress on `/api/calendar` for HMAC-token-gated `.ics` feed. Dedicated single-replica Deployment `tripit-mail-listener` holds an IMAP IDLE subscription and immediately reconciles bookings forwarded to plans@viktorbarzin.me (catch-all → spam@); `tripit-ingest-plans` remains scheduled `*/15` as the repair path. Both paths poll read-only, share one PostgreSQL advisory lease, route ONLY to a registered user / verified linked address (no default-owner fallback; strangers ignored), parse with local LLM (`qwen3vl-8b`), and email the sender the outcome (Added to trip / Couldn't import). Plus `tripit-poll-flights`, `tripit-run-reminders`, `tripit-watcher-sync`, `tripit-transport-nudge`, `tripit-weather-brief`, `tripit-fill-tour-audio` (and its `-retry`), and `tripit-sync-country-days` (nightly 03:40 London, tripit ADR-0060: syncs each user's own linked Dawarich Location history into Travel year Country days, using a per-user key sealed under `DOCUMENT_ENCRYPTION_KEY` and never the global `DAWARICH_API_KEY`. `DAWARICH_URL` turns Location history on and is the default Dawarich users see (tripit's `DAWARICH_URL` ADR); a user may link their own public Dawarich instead. Requests to that origin, from the Deployment, this job and the two nudges, go in-cluster via `DAWARICH_INTERNAL_URL` plus `Host: dawarich.viktorbarzin.me`, because the bare svc 403s on Rails host authorization and the public host 504s at Traefik's 30s `responseHeaderTimeout`). (The old Gmail-scrape `tripit-ingest-mail` CronJob was removed 2026-06-05.) App secrets in Vault `secret/tripit`. | tripit |
...
| orchestrator-sandbox | **TEMPORARY (2026-09-24)**, for the orchestrator bake-off (plan `~/code/docs/agents/2026-09-24-orchestrator-bake-off.md`, in the monorepo root rather than this repo). Namespace `orchestrator-sandbox` where Scion runs its agent pods: Pod Security Admission `baseline` (no privileged pods, host namespaces or hostPath), a quota of 24Gi memory on requests and limits and 12 CPU on requests, with a matching LimitRange (no `limits.cpu`: Kyverno `strip-cpu-limits` removes CPU limits cluster-wide, so such a quota would reject every pod), and a `scion-broker` ServiceAccount whose namespace-only Role covers pods, exec/attach/log, secrets, configmaps and PVCs. Scion's hub on the devvm uses its long-lived token (`scion-broker-token`). Agent pods read `CLAUDE_CODE_OAUTH_TOKEN` from the `claude-oauth` Secret, an ExternalSecret of Vault `secret/orchestrator-sandbox` (wizard's own token, not shared with the fixer or claude-agent-service). Removed after the sprint unless Scion wins; teardown steps are at the top of `stacks/orchestrator-sandbox/main.tf`. | orchestrator-sandbox |
