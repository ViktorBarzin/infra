...
| 46 | Immich Smart Search | `clip_index` residency in PG `shared_buffers` + representative ANN probe latency (in immich-postgresql). FAIL >1.5s or <50% resident; WARN >0.5s or <90% resident. Cold cache → check the `immich-search-probe` CronJob (it runs the prewarm on :00/:30 ticks since 2026-08-16, when clip-index-prewarm + clip-keepalive were merged into it) |
