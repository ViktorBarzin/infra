...
- **Database rotation**: Vault DB engine rotates passwords every 7 days (604800s). MySQL: speedtest, wrongmove, codimd, nextcloud, shlink, grafana, phpipam. PostgreSQL: health, linkwarden, affine, woodpecker, claude_memory, crowdsec, technitium, paperless_ngx. Excluded: authentik (PgBouncer), root users. **Apps that read a rotated secret only at startup** (env var / initContainer, not a hot-reloaded mount) MUST carry a Reloader annotation (`secret.reloader.stakater.com/reload: <secret>`) or they keep the stale password and silently fail DB auth on each rotation until manually restarted — matrix's Synapse `inject-db-password` initContainer hit exactly this (found via Loki 2026-06-05, ~12.9k auth-fail lines/hr); matrix has since migrated to tuwunel (RocksDB, no Postgres) on 2026-06-08 and is no longer in the rotation list above. **The alternative that avoids the restart entirely** (f1-stream, 2026-09-26): mount the ESO-generated Secret as a FILE, template it with `reloader.stakater.com/ignore: "true"` so a Deployment carrying `reloader.stakater.com/auto` does not roll on each rotation, and have the app read the password from the file for every new connection (asyncpg takes a password callable). Pin the static role to a `rotation_schedule` for the gap before ESO's next refresh, when new connections fail but pooled ones keep working. See `stacks/f1-stream/main.tf` (`viewing_db_external_secret`) and `stacks/vault/main.tf` (`pg_f1_stream`). Technitium uses a password-sync CronJob (every 6h) to push rotated password to the Technitium app config via API, disable SQLite + MySQL logging, check PG plugin is loaded, configure PG query logging (90-day retention), and disable SQLite on secondary/tertiary instances.
