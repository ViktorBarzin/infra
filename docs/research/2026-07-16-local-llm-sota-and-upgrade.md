...
| **qwen3.8-27b** (round 3, 2026-08-21, llama.cpp **b10524**) | The DeltaNet revisit below, run. **UD-Q2_K_XL: 8.2 tok/s gen / 113 tok/s prefill**, output coherent and accurate; **UD-IQ1_S: 8.5 tok/s** but unusable quality (repetition loops, as its 76.3% top-1 predicts). Resident 9670 MiB — fits only with immich-ml scaled to 0. Still ~16% of the card's bandwidth ceiling, so the Turing DeltaNet path is now *correct* but not yet fused. | **REMOVED**. 8 tok/s vs qwen3-8b's 33, plus a co-tenant eviction, is not worth it for triage/enrichment. |
...
- ~~**Revisit qwen3.5** when llama.cpp lands an optimized `qwen3_5` CUDA path~~ — **DONE 2026-08-21** (see the qwen3.8-27b row above). The follow-up was the right one to queue: b10524 took the same DeltaNet family from 0.5 to ~8 tok/s, so this was a support-maturity problem as expected. It is now a *partly* solved one — the path is correct on Turing but unfused, so it lands at ~16% of the bandwidth ceiling instead of the 60–80% a mature CUDA path reaches. Worth revisiting again if a fused Turing DeltaNet kernel lands upstream (`ik_llama.cpp` already ships one, which is where to watch). Note for whoever runs it: a wrong/older build fails by emitting **garbage tokens rather than an error**, so pin the engine digest before judging any quant ([llama.cpp #27164](https://github.com/ggml-org/llama.cpp/discussions/27164)).
