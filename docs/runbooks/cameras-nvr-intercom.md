...
`.203` е в предишното състояние: отговаря на порт 80 на всеки адрес от мрежата и
приема `admin`. Същото ограничение там не е сложено. `.202` е с по-нов фирмуер
...
Остава: часовникът е с час назад, защото лятното време е изключено (същото като
при монитора `.134`), и препращането 8899 сочи към `192.168.0.100`, който е
първият адрес от DHCP пула - работи само ако логърът хване точно него.
