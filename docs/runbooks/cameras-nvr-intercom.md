...
### Канал 25 след сервиза - защо не се вдигна и с какво се вдигна (03.09.2026)

Панелът `.115` се върна на входа на 02.09.2026 20:38:58, а регистраторът го
държеше офлайн. Мрежата не беше причината:

| Проверка от `guestns` | `.115` | `.116` за сравнение |
|---|---|---|
| ping | 0% загуба, ~4 ms | 0% загуба |
| портове 80 / 554 / 8000 | отворени | отворени |
| ONVIF `GetSystemDateAndTime` без парола | HTTP 200 | HTTP 200 |
| ISAPI `System/deviceInfo` с `admin` + новата парола | 200, `VTORI VHOD` | 200 |
| **ONVIF `GetDeviceInformation` с WS-Security** | **`ter:NotAuthorized`** | **200, DS-KD9203-E6** |

Тоест устройството приема паролата по ISAPI и по уеб интерфейса, но **ONVIF я
отхвърля** - а каналите на панелите в регистратора бяха добавени точно по ONVIF.
Пробвани са и старата парола на панелите, и потребител `onvif`: пак
`NotAuthorized`. Списъкът с ONVIF потребители се води отделно от потребителите
на устройството (`/ISAPI/Security/users` показва само `admin` и на двата панела)
и на този модел не се управлява по ISAPI - `/ISAPI/Security/ONVIF/users`,
`/ISAPI/System/Network/Integrate` и подобните връщат `notSupport`, тоест не е
въпрос на права. Най-правдоподобното обяснение е, че сервизът е изчистил ONVIF
акаунта заедно с паролата.

**Решението беше да не се поправя ONVIF, а да се смени протоколът на канала.**
Панелът е Hikvision устройство и порт 8000 (SDK) е отворен, затова канал 25 е
пренасочен от ONVIF:80 към HIKVISION:8000 със същия `admin` и новата парола:

```sh
PUT /ISAPI/ContentMgmt/InputProxy/channels/25
  proxyProtocol=HIKVISION, managePortNo=8000, userName=admin, password=<secret/emo/vermont-panel-115>
```

Резултатът е потвърден, а не изведен: каналът стана `online=true` с потоци
2501/2502, регистраторът научи `model=DS-KD9203-E6` и серийния номер (празни
преди това - те се учат от устройството при първи успешен логин), снимка от
`/ISAPI/Streaming/channels/2501/picture` върна истински JPEG, а след презареждане
на `hikvision_next` в HA се появи `camera.ds_7632nxi_..._2501`, което отдава
кадър 1280x720. Мониторът на Pi-то отчита **25 живи от 27**.

**Живото видео работи; за записа още няма присъда (03.09.2026).** Настройките на
канал 25 съвпадат с тези на работещия канал 26: същият график, тригер `VMD-25` с
връзка `record-25`, детекция на движение включена с еднакъв grid (`fffffc...`) и
чувствителност (единствената разлика е `PostRecordTimeSeconds` 60 срещу 30).
Часовниците на .115 и .116 показват еднаква зона и вярно местно време, тоест и
това отпада като разлика.

За първия час на линия каналът има нула записа. Това само по себе си не е
дефект: за 24 часа броят записи по панелите е 42 (кан.21), 20 (22), 35 (23),
**2 (24)** и 76 (26), тоест един панел спокойно стои с два записа на денонощие и
нула за час е в нормата.

ПОУКА ОТ ТАЗИ ПРОВЕРКА, която едва не влезе тук като извод: ръчният запис
(`PUT /ISAPI/ContentMgmt/record/control/manual/start|stop/tracks/<track>`) връща
`OK`, но НЕ оставя намираем клип - и на канал 25, и на работещия канал 26. Тоест
той не става за проба на записа; „пуснах ръчен запис и няма нищо" не доказва
нищо. Единствената валидна проверка е да се изчакат няколко часа и да се търси
пак с `POST /ISAPI/ContentMgmt/search`, чиято работоспособност се доказва с
позитивна контрола върху канал, за който се знае, че записва.

Наследство, което остава: другите пет панела продължават да минават по ONVIF.
Ако на някой от тях бъде сменена паролата или ремонтиран модул се върне със
изчистен ONVIF, същият номер важи - каналът се прехвърля на HIKVISION:8000,
вместо да се гони ONVIF потребител, който този модел не дава да се редактира
отдалечено.
