...
(собствен `/etc/nvr-channels.env`, затова демонтажът не го засегна). Устойчиво
**24 живи от 27**, тоест картичката `nvr_vermont` в алармата стои **amber** —
това е нормалното състояние, а не нова авария: долу са камери `.52`/`.53` и
канал 25 = панелът `.115`. Канал 28 не е камера и не се брои.

От **02.09.2026** `.115` вече не е на сервиз: върнат е на входа (стартиран
20:38:58), пингва от `guestns` с 0% загуба, портове 80/554/8000 са отворени и
ONVIF услугата му отговаря. Регистраторът обаче продължава да държи канал 25
`online=false` и без `model`/`serialNumber`, тоест още не е говорил с него -
виж „Канал 25 не се вдига след сервиза" по-долу.
...
### Канал 25 не се вдига след сервиза (мерено 03.09.2026)

Панелът `.115` е върнат на входа на 02.09.2026 20:38:58, а регистраторът още го
държи офлайн. Какво е проверено от `guestns` на rpi-sofia и какво показва:

| Проверка | `.115` | `.116` (за сравнение) |
|---|---|---|
| ping | 0% загуба, ~4 ms | 0% загуба |
| портове 80 / 554 / 8000 | отворени | отворени |
| ONVIF `GetSystemDateAndTime` без парола | HTTP 200, 2428 B | HTTP 200, 2431 B |
| канал в NVR-а | `online=false`, `model` и `serialNumber` **празни** | `online=true`, `DS-KD9203-E6` |

Конфигурацията на канала е същата като на работещите: `ONVIF`, порт 80,
потребител `admin`, `streamType=auto`. Тоест мрежата и ONVIF услугата на панела
са наред, а регистраторът не се е логвал в него нито веднъж - празните
`model`/`serialNumber` се учат от устройството при първи успешен логин.

Остават две обяснения, и двете от страна на автентикацията, която ние не можем
да пробваме (за панелите нямаме валидна парола - виж таблицата с 401 по-горе):

1. Паролата, записана на канал 25 в NVR-а, вече не съвпада - сервизът е
   активирал панела наново с друга.
2. Списъкът с ONVIF потребители на панела е изчистен при сервиза. ONVIF услугата
   отговаря и без парола на `GetSystemDateAndTime`, така че тестът по-горе не
   различава двата случая.

Кое от двете е, се разбира само с влизане в панела или в канала на NVR-а с
инсталаторска парола. Действието и в двата случая е едно и също: паролата на
канал 25 да се въведе наново в регистратора (или ONVIF потребител да се създаде
пак на панела) - промяна по общото оборудване на комплекса, тоест за управата.

Дотогава: панелът работи като домофон (`panel115-watch` го чете нормално), но
видеото от втори вход не влиза в регистратора и няма запис от него.
