...
**Живото видео работи, записът - не (към 03.09.2026).** Настройките на канал 25
съвпадат с тези на работещия канал 26: същият график, тригер `VMD-25` с връзка
`record-25`, детекция на движение включена с еднакъв grid (`fffffc...`) и
чувствителност (единствената разлика е `PostRecordTimeSeconds` 60 срещу 30).
Въпреки това за 45 минути на линия каналът има нула записа, докато за 24 часа
другите панели имат 42 (кан.21), 20 (22), 35 (23), 2 (24) и 76 (26). Ръчен запис
(`PUT /ISAPI/ContentMgmt/record/control/manual/start|stop/tracks/2501`) връща OK
и пак не оставя нищо - тоест пътят към диска е затворен, а не липсва движение.

Най-правдоподобната причина е режимът на диска: `/ISAPI/ContentMgmt/Storage`
показва `workMode=quota`, тоест всеки канал има отделна квота в GB, а канал,
който никога не се е свързвал, най-вероятно е с 0 GB. Квотата не се чете и не се
пише по ISAPI на този модел - `Storage/quota`, `record/quota` и роднините връщат
`notSupport`. Остава интерфейсът на самия регистратор или iVMS, тоест намеса по
общото оборудване.
