...
| 6 | **Backup-MX drain** — the pfSense WireGuard *package* regenerates `tun_wg0.conf` on boot; mx2 had been added only as a hand/live peer (not a package peer), so the outage reboot dropped it and broke the drain (~16 emails deferred on mx2, not lost). **DONE 2026-07-18**: re-added the peer + drained the queue, then promoted mx2 to a proper **WireGuard package peer** (config.xml, like the site peers) so it is regenerated into the conf and survives reboots natively — no hook needed. | `docs/runbooks/backup-mx.md` | done |
