...
**Single source of truth:** `infra/scripts/workstation/roster.yaml` (`os_user → authentik_user / k8s_user / tier / namespaces`). `roster_engine.py` (pytest-covered pure core) derives desired state; `t3-provision-users` (hourly timer) applies it — **additive-only** for existing users (never strips a group, replaces a home, or re-locks an account). `/etc/ttyd-user-map` + `/etc/ttyd-admins` + `dispatch.json` are **generated** from the roster (do not hand-edit). **(2026-08-17, two changes in one day. (a) The roster is the ONLY place the identity map is stored — terminal-lobby carried a second copy at `devvm/ttyd-user-map` which its `deploy.sh` installed over the generated file on every run, and by then it had drifted and would have dropped `ancaelena98=ancamilea`; that copy is deleted and terminal-lobby only reads the generated map. (b) **Authentik's `T3 Users` group is now the source of truth for WHO has devvm access**, and `roster.yaml` is the policy table keyed by user: `.woodpecker/t3-membership-sync.yml` (cron, 15 min) reconciles rows to match membership — writing a floor-tier row (`namespace-owner`) for a new member and commenting out the row of anyone who left — and the devvm reconcile applies the account work from the **committed** roster, including the reversible cut (`offboard_plan`: disable the per-user units, lock the login; never `userdel`). Adding or removing someone is one action in Authentik. Two things stay deliberately manual: raising a user above the floor tier (a reviewed roster commit, still cross-checked against Vault `k8s_users`), and terminal-lobby's per-user sudo grant in `devvm/sudoers.d-ttyd-users` — without it a new user reaches the lobby but cannot attach a terminal.)**
...
**Config inheritance (live, reworked 2026-09-22):** every user has one instruction file of their own, in the AGENTS.md format that Claude Code and Codex both read, and one layer is shared. The shared layer is the org `claudeMd` in `scripts/workstation/managed-settings.json`: `sync_managed_config` installs it to `/etc/claude-code/managed-settings.json` (top precedence in every Claude session), and `sync_codex_requirements` writes the same text into `/etc/codex/requirements.toml` as `additional_developer_instructions`, which Codex sends as a developer message (validated with `tomllib`, renamed into place). Since 2026-09-25 `sync_pi_org_policy` also writes the bare text to `/etc/pi/org-policy.md` for pi, the third harness in terminal-lobby, whose lobby extension appends it to pi's system prompt on every turn. The per-user file is `~/.agents/AGENTS.md`, read-only, and reaches the two harnesses through two symlinks, `~/.claude/CLAUDE.md` and `~/.codex/AGENTS.md`; neither harness reads `~/.agents/` on its own. Pi reads it through a third link, `~/.pi/agent/AGENTS.md`, which `install_pi_links` makes for the users `install_user_agents` manages (agents-sync makes wizard's); the same step links terminal-lobby's pi extension into every roster user's `~/.pi/agent/extensions/terminal-lobby.js`, and leaves a file or link the user put there in place. Viktor writes the files for emo and ancamilea in the monorepo at `docs/agents/users/<user>/AGENTS.md`: `materialise_user_agents` reads them from the monorepo's `origin/master` into the root-only `$STATEDIR/agents`, and `install_user_agents` installs each one as its user (through `runuser`), refusing a symlinked `~/.agents`, `~/.claude` or `~/.claude/rules`. On a fetch failure the last good copy stays in use. Each user also keeps `~/.claude/rules/99-personal.md`, which the provisioner creates once and never overwrites. wizard's file is built on the box by `agents-sync`, a 15-minute systemd user timer, from `~/.agents/core.md` plus `~/.agents/personal.md`; the provisioner leaves it alone. Since 2026-09-25 `~/.agents` is where he edits: chezmoi manages it from `dot_agents/` in his dotfiles, and agents-sync saves edits made there back to the repo. Every skill he has lives in `~/.agents/skills`, which Codex reads directly, with links from `~/.claude/skills`; the skills updater installs his upstream skills for both harnesses (`playbooks/files/devvm/claude-skills/wizard/agents`). breakglass gets the org policy only. Claude's local auto-memory is off for everyone (`autoMemoryEnabled: false` and `CLAUDE_CODE_DISABLE_AUTO_MEMORY=1` in managed settings), since memory lives in `homelab memory`. This replaced three earlier mechanisms: `install_shared_rules`, which copied `docs/agents/shared/*.md` into every `~/.claude/rules/`; `refresh_codex_mirror`, which copied the claudeMd into each `~/.codex/AGENTS.md`; and the `/etc/skel/.claude/{rules,skills}` symlinks into wizard's home, which would have given the next roster user ownership of his rules directory. Design: monorepo `docs/agents/2026-09-22-per-user-agent-instructions.md`; layout: monorepo `docs/agents/users/README.md`. Secrets stay per-user at mode 600, never symlinked. **(2026-06-26: the managed `claudeMd` now defers OS-level file access to the OS/sudo — a user holding broad `sudo` may read other users' files incl. `~/.claude`; the mode-600 / no-symlink posture is unchanged but is no longer reinforced by an agent "never read other homes" rule. See [ADR-0015](../adr/0015-os-is-the-authorization-boundary.md).)** **The managed config self-deploys from the repo** (2026-06-10): the hourly reconcile's `sync_managed_config` installs `scripts/workstation/managed-settings.json` to `/etc/claude-code/` whenever the repo copy changes, so editing the claudeMd is an edit and a commit, with no manual install. Since 2026-08-22 it reads infra's `origin/master` copy (step 0a fetches once, as the checkout's owner) and falls back to the admin's working tree only when that fetch fails, so landing the change is enough; before that it read the working tree, which could sit behind master for days. The roster and the provisioner script itself come from `origin/master` the same way. Repo-level guidance (each repo's `AGENTS.md`, with a `CLAUDE.md` symlink beside it, and `CONTEXT.md` in the infra repo) reaches non-admins through their auto-freshened clones — commit + push and every user has it within the hour. **(2026-07-07: managed-settings.json also carries org-wide Claude Code hooks that run `/usr/local/bin/claude-tmux-state` — deployed by the terminal-lobby repo — stamping `@claude_state` on the enclosing tmux session so the web terminal sidebar shows running / awaiting-input / completed per session; the script no-ops outside tmux, so t3-serve and headless sessions are unaffected. See terminal-lobby `docs/adr/0001-claude-state-via-hooks.md`.)**

**Seeing and editing it (2026-09-26):** agentmd (`agentmd.viktorbarzin.me`, source github.com/ViktorBarzin/agentmd) shows a user their own instruction files, which harness loads which file in each folder (it probes Claude Code and Codex against a stand-in API, so no model is called), and where the files repeat or contradict each other, with an editor that writes in place and can commit and fast-forward push. The org policy appears as a read-only copy that opens `scripts/workstation/managed-settings.json` for editing (`/etc/agentmd/config.toml`). Each user runs their own instance as themselves, `agentmd@<user>` on the devvm behind its own hostname and proxy secret (`stacks/agentmd`, `devvm_agentmd_users` in `playbooks/devvm.yml`); wizard has one so far.

**Model selection lives in ONE place (2026-08-01):** the `model` key in `managed-settings.json` (→ `/etc/claude-code/managed-settings.json`) chooses the default Claude model for **every** user — currently `claude-opus-5-5` (raised from `claude-opus-5` on 2026-09-22, the day Opus 5.5 became entitled to this account). Per-user `model` keys in `~/.claude/settings.json` were all removed (emo's `opus[1m]` was the last) and should not be re-added: managed `model` is a **default, not a lock** (it sets no `availableModels`/`enforceAvailableModels`), so a personal key silently OUTRANKS it and diverges that user from the fleet with no signal — precedence is `--model` flag > per-session `/model` > user `settings.json` > managed > built-in. Per-session `/model` is the intended escape hatch and still works, but on Claude Code 2.1.280 it does **not** stay per-session: choosing a row writes that slug into `~/.claude/settings.json` and says so ("Set model to `claude-opus-5-5` and saved as your default for new sessions"), and it fires even when the picker is opened only to look and the selection does not change (measured 2026-09-22). So a personal pin now arrives by keystroke rather than by hand edit, and clearing it is a deliberate step: delete the `model` key, which the CLI appends last. To read the current model without writing one, use the startup banner, which names the model and its source ("Using `claude-opus-5-5` (from managed settings)"). **Convention, not enforcement** (Viktor, 2026-08-01): nothing strips a re-added personal key, so when a user's model looks wrong, grep their `settings.json` first. Two traps when changing the value: prefer the plain id over an alias (`opus` still resolved to `claude-opus-4-8` for a day after `claude-opus-5` became available), and remember that an unentitled model does NOT error — it silently falls back (requesting `claude-opus-5` on 2026-07-31 served Opus 4.8, which is why that first attempt was reverted). Verify a new default from live data, not the config: the served model is recorded per assistant message in `~/.claude/projects/<slug>/<session>.jsonl` (`"model":"…"`), and `~/.claude.json` → `modelAccessCache` carries per-model `entitled` flags for the account.

**Effort defaults to high for everyone (2026-09-25):** the same file sets `"effortLevel": "high"`, at Viktor's request that every user default to Opus 5.5 at high and nobody to max. Claude Code 2.1.282 saves a user's chosen effort per model under `modelSettings.<model>.effortLevel` (where `/effort` writes it), but for the model the policy pins it reads effort only from the policy and from flags, so a user's own `effortLevel` or `modelSettings` entry for `claude-opus-5-5` is ignored. Measured with a local fake API: before this key existed, a session started without a flag ran at `medium`, the model's own default, whatever the user's `settings.json` said. An explicit `--effort` or `/effort` still sets one session's level, and `max` can only be chosen that way, since `effortLevel` does not accept it. `maxEffortLevel` in the same file would cap every source if a hard limit is ever wanted. Launchers that pass flags bypass the default: Terminal Lobby's per-user new-session choice (`session.newModel` and `session.newEffort` in `/var/lib/tmux-api/prefs/<user>.json`, `default` means no flag), a user's `~/.config/terminal-lobby/commands`, and suspend/resume, which re-runs a session's original command line. Check what a live session really runs with `tr '\0' ' ' < /proc/<pid>/cmdline`, not the settings files.

**The `/model` picker lists exact slugs, from the same file (2026-09-06):** the `modelPicker` key beside `model` replaces Claude Code's four built-in family rows (Default, Sonnet, Opus, Haiku) with one row per slug — `claude-opus-5-5`, `claude-opus-5`, `claude-opus-5[1m]`, `claude-sonnet-5`, `claude-haiku-4-5-20251001`, `claude-opus-4-8` — because a family word cannot name the thing a person is choosing between: `claude-opus-5`, `claude-opus-5[1m]` and `claude-opus-4-8` are all "opus". The shape is `{ replaceBuiltInOptions, options: [{ model, label, description, behavesAs }] }`, and `behavesAs` is how a slug this build's catalogue does not know is mapped onto one it does. Three things measured on 2026-09-06 against Claude Code 2.1.263, each of which would otherwise cost an afternoon. It is honoured from MANAGED settings only — the same block in a user or project `settings.json` is dropped silently, so this file is the only place to change it. `replaceBuiltInOptions: true` does NOT remove the "Default (recommended)" row, which stays as row 1, so nothing is lost by replacing rather than appending. And a row the account is not entitled to is dropped from the rendered list rather than shown and refused. Only models this account can actually run are declared here (Viktor, 2026-09-06): `claude-fable-5` was in the list for a few hours and came out, because it is not entitled and `--model claude-fable-5` starts a Sonnet 5 session with a warning rather than failing. The account's own answer is the `modelAccessCache` array of `{apiName, entitled}` in `~/.claude.json`; confirm a new row against that and then against the startup banner, never with `claude -p`, which hides the warning. The rows double as terminal-lobby's model list: its chip on a live session drives this very picker and matches a row by its LABEL (`sessionio/setmodel.go`), so the two lists have to stay in step (`frontend-v2/src/lib/models.ts`).

**Auto-mode classifier context (2026-08-16):** `managed-settings.json` also carries an `autoMode` block. Claude Code's auto mode routes each tool call past a classifier, and `claude auto-mode defaults` showed every environment slot as `None configured` — so the classifier judged this homelab with no idea what it is. With nothing describing the place, an operator reading their own Vaultwarden item looks like unexplained credential access and an internal host looks like an exfiltration target: over one week that blocked 25 Bash calls, clustered on exactly the credential steps a camera/router/HA diagnostic needs (`homelab vault kv list secret/<user>`, `homelab vault get <item> --field password`, `homelab ha token`). This is [ADR-0015](../adr/0015-os-is-the-authorization-boundary.md)'s principle one layer up: Vault and Vaultwarden already enforce per-user authorization server-side (creds at `secret/workstation/claude-users/<user>`; kv reads use the caller's own OIDC token), so the classifier was re-litigating a decision the service had already made. The `environment` entries name the org, the `homelab` CLI's read verbs, both secret stores, the internal domains, the RFC1918 LANs, source control, CI/CD, and the IaC scopes. **Prose, not `permissions.allow`, is deliberate:** a prefix rule like `Bash(homelab vault get *)` never matches the compound shapes these actually take (`PW=$(homelab vault get …)` piped into ssh), which is most of them; `autoMode` entries are natural-language and shape-independent. The block is **net tighter than the starting point** — `claude auto-mode critique` flagged that the first draft's two `allow` rules were broad enough to silently clear soft blocks on the firewall, door locks, and DNS, and that describing CI-applies-on-push as context left it unguarded; both were fixed, and six homelab `soft_deny` rules now cover infra auto-apply, network devices, physical access/alarm, public DNS, cluster mutation outside Terraform, and household media deletion (red-lines that previously lived only as prose in the claudeMd). **Every array must keep the literal `"$defaults"`** or it replaces the built-in list wholesale — omitting it from `soft_deny` would discard all 66 built-in rules including force-push and `curl | bash`. Verify any edit with `claude --settings <file> auto-mode config` (expect 32 environment / 19 allow / 72 soft_deny / 1 hard_deny) and review custom rules with `claude auto-mode critique`. Deployment follows the same `WORKSTATION_DIR`-reads-the-admin's-tree caveat noted above.
...
**Memory — an external key, and provenance in the recall block (2026-09-16):** the memory
API's keys now carry a scope. `API_KEYS` still parses the flat `{"user": "key"}` shape as
`admin`, and additionally accepts `{"muse": {"key": "…", "scope": "external"}}`. An external
key reaches only ten allowlisted operations, cannot call `import`, `migrate-secrets`,
`/api/users` or the per-id secret endpoint, and cannot reach `/mcp/*` at all; every write it
makes is stamped `source:<user_id>` server-side, which the client can neither remove nor
forge. The reason this matters at the workstation layer: `homelab-memory-recall.py` injects
its results into every turn of every session on the box, so an entry written by an outside
assistant is indistinguishable from one Viktor wrote unless something says so. The hook now
renders that tag inline, `#13293 [facts] [via muse] (0.20) …`, and leaves untagged entries
exactly as before. It is generic in the agent name, so a second external key needs no change
here. Note reads stay GLOBAL — recall, list and get carry no owner predicate, so an external
key reads every user's entries, which is a recorded decision rather than an oversight
(`docs/plans/2026-09-16-muse-memory-connector-design.md`). First consumer is Meta's Muse,
over the public Cloudflare path; it holds no tailnet node.

**zsh guard — a PreToolUse hook for two shell semantics agents keep tripping on (2026-08-15):** Claude Code's Bash tool runs each user's login shell, which here is zsh, and there is no setting to change that (`defaultShell` only covers input-box `!` commands; the Bash tool follows the passwd shell, visible as the `snapshot-zsh-*` files under `~/.claude/shell-snapshots/`). Agents write bash-idiomatic shell, and two zsh differences fail quietly rather than loudly: zsh does **not word-split** unquoted expansions, so `F='-a -b'; cmd $F` passes ONE argument and the command sees a single malformed flag (this is the git-crypt filter-flag shape that recurred here for months); and zsh **globs** an unquoted `word(...)`, so an unquoted conventional-commit subject like `fix(scope): x` raises "no matches found" and expands to nothing, silently taking the wrong commit subject. Both had been written to memory repeatedly and still recurred, because memory cannot intercept a command. `claude-hooks/zsh-guard.py` is wired as `PreToolUse` with `matcher: Bash` by the same `install_memory` reconcile step as the memory hooks, and exits 2 with the remedy attached. It is deliberately narrow — it fires only when a variable holding a whitespace-containing **flag list** is expanded unquoted, or a bare `word(` appears outside quotes — and it skips quoted values, arrays, `${=VAR}`, heredoc bodies (so the repo's own `git commit -F - <<'EOF'` convention is never blocked), arithmetic, command substitution and subshells. Calibration evidence: replaying 202 real Bash commands from one working session blocked exactly one — the command that had actually failed. Any exception inside the guard exits 0, so a bug in it can never block real work. Tests: `claude-hooks/test_zsh_guard.py` (25 cases).

**Writing-style check — a Stop hook, because rule text does not reach a generation reflex (2026-09-02):** `claude-hooks/unslop-check.py` reads the finished reply and asks for a rewrite when a mechanical tell survives: an em dash, a curly quote, one of ~20 banned words, a chatbot phrase, a bold-label bullet, or more than 300 prose words. The rules it enforces are the writing-style section of each user's own AGENTS.md (see config inheritance above; until 2026-09-22 the shared `~/.claude/rules/40-style.md`); the hook is installed and wired by the same `install_memory` step as the memory and zsh hooks. Why a hook and not the rule alone: measured across 1,331 transcripts, the `unslop` skill that held these rules was invoked 7 times in 14 days, and once the rules moved into always-loaded `CLAUDE.md` text the em-dash ban was present in every session and still produced 6,068 em dashes over 7,302 replies in a week, while the vocabulary rules held at 14 hits. So the failure is one reflex, not a knowledge gap. `stop_hook_active` bounds it to one retry per turn, and any unparseable input exits 0, so it cannot wedge a session. Two deliberate exemptions: fenced code, inline code and blockquotes are stripped before matching, since a pasted log line is not the model's own prose (that covered all but 1% of measured hits), and replies written mostly in Cyrillic keep their dashes, because in Bulgarian and Russian the dash is standard punctuation and the check would otherwise fight correct grammar. One implementation note worth keeping: the reply comes from the Stop payload's `last_assistant_message`, since at Stop time the assistant record is not yet flushed to the transcript, and a transcript read returns an empty string with every check passing silently. Tests: `claude-hooks/test_unslop_check.py` (25 cases).

**Agent skills — self-service between users (2026-08-19, replacing the vendored allowlist):** each user's skills live in their own `~/.claude/skills`, and moving one between accounts is now a user action in the lobby's Settings overlay rather than a reconcile step. The **Skills group** (terminal-lobby `docs/adr/0011-skills-move-between-users-by-copy.md`, backed by `skills-api` on `:7688`) lists what an account loads, what every other terminal account has with a `same`/`differs` verdict, and installs a **copy** into `~/.claude/skills/<name>` with its provenance recorded in `.manager.json`. Visibility is total and symmetric, which grants nothing new — `/home/wizard` is `751` with `.claude/skills` at `775`, so those files were already readable, and the admin reads a `0700` home through the sudo they already hold. Because peer homes are `0700` in the other direction, an install runs as **two** users: `skills-api` packs the skill in a privileged child as the owner and unpacks it in another as the recipient, so it works both ways rather than only admin→user.

*What this replaced:* until 2026-08-19 the reconcile's `install_skills` gave users on a `SKILL_USERS` allowlist (`emo`) their own copies of a 17-skill snapshot vendored at `scripts/workstation/claude-skills/`, copied into `~/.agents/skills/<name>` with a relative symlink from `~/.claude/skills/`. That reliably got a starter set onto the box; two things it did not do were refresh a copy after its first install, and let each person choose their own set. The function, the allowlist and the snapshot were removed together; **existing copies on disk were left exactly as they were**, so emo keeps all 21 of his and wizard can install any of them from him. The `~/.agents/skills` indirection has no remaining consumer — nothing on the box reads it — but the symlinks still resolve and the manager treats such an entry as an ordinary skill.
...
**Per-user browser MCP — playwright, reproducible from git (2026-06-16):** every user (incl. the admin) gets their OWN isolated `@playwright/mcp` server so their concurrent Claude sessions don't fight over tabs (`--isolated` → a fresh browser context per MCP connection), wired into Claude in **every directory** via a user-scope `~/.claude.json` entry (`playwright → http://localhost:<PLAYWRIGHT_PORT>/mcp`). Mechanism: **system-level template units** `playwright-mcp@<user>.service` + `playwright-snapshot-refresh@<user>.{service,timer}` (`User=%i`, sourced from `scripts/workstation/playwright/`, installed by `setup-devvm.sh` §9e — system manager, so NO systemd --user / linger). `roster_engine.py` allocates a sticky per-user `PLAYWRIGHT_PORT` (`PLAYWRIGHT_BASE_PORT=8931`); the reconcile's `install_playwright()` writes it, seeds the chrome-service snapshot token if-absent (staged from Vault `secret/chrome-service` to `/etc/t3-serve/chrome-service-token` by `setup-devvm.sh` §8c, since the hourly root reconcile has no Vault token), wires `~/.claude.json` by running `claude mcp add --scope user` AS the user (clobber-proof + if-absent, so it fixes existing/new/admin without rewriting a populated config), and `enable --now`s the instances (idempotent — never restarts a running server). The `@playwright/mcp` version is **pinned** in the unit (the `@latest`-silently-rolls-the-fleet footgun — see `T3_PIN`). Replaced the earlier hand-made `~/.config/systemd/user/playwright-*` units (pre-migration emo/anca had servers running but never wired into their `.claude.json`). **The 2026-06 migration enabled the new instances but did NOT retire the old ones**, and that gap went unnoticed until 2026-08-14: the legacy user units kept running with their own HARDCODED `--port`, so every system instance crash-looped on `EADDRINUSE` from 2026-07-29 onward — `NRestarts` 184,967 (wizard) / 185,181 (emo) / 187,422 (ancamilea), i.e. **all three users had no working browser MCP for ~16 days**. The ports were also cross-wired (wizard's legacy unit held ancamilea's 8931 and hers held wizard's 8933), which makes the symptom read as one user squatting on another's port when in fact each was blocked by the fleet's own leftovers. `install_playwright()` now calls `retire_legacy_playwright_units()` first — stop, disable, and rename each legacy unit to `<unit>.superseded-by-system-unit` (renamed rather than deleted, so the retirement is visible and reversible) — before enabling the template instance; idempotent, so an already-migrated or never-affected user is a no-op. Cookie-warming pipeline + ops: `../runbooks/chrome-service-snapshot.md`.
...
**Web-terminal session persistence (2026-06-10):** the tmux-based web terminal's named sessions (each running one Claude conversation) survive devvm reboots — `tmux-persist-save.timer` (5-min) snapshots every terminal user's sessions (name, cwd, conversation uuid), and `tmux-persist-restore.service` recreates missing sessions at boot with `claude --resume <uuid>` (per-session idempotent). Since 2026-08-14 the store is a **series** under `/var/lib/tmux-persist/snapshots/<user>/<ts>.tsv` rather than one rewritable manifest, written only when the session set changes and pruned to the newest 200 (~3 weeks at the measured ~10 changes/day); `<user>.tsv` is a symlink to the newest, so the boot path and the manifest cannot disagree. That exists because a **partial** loss — the tmux server alive while the processes inside sessions are killed — used to be overwritten by the next save before anyone could restore from it (2026-08-14 and 2026-07-18). A deliberate kill is now recorded as a tombstone in `<user>.forgotten.tsv` rather than by deleting a row, since deleting one would erase the session from every older snapshot too. The conversation uuid is read straight from the claude process's argv — the `--session-id` that `start-claude.sh` pins on every fresh launch (added 2026-07-26), or the `--resume` a restore carries — so it is correct **per-process**. This replaced a "newest transcript by mtime in the cwd-slug dir" guess that mis-attributed concurrent sessions: since a user's sessions all share ONE project dir (all cwd'd in `~/code`), the guess collapsed many session names onto whichever conversation was most active, so a restore reopened every tab on the SAME conversation (emo, up to 11/13 tabs — root-caused + fixed 2026-07-26; `uuid_of_claude` keeps a name-matching then mtime fallback only for a session started without either flag). The binary self-deploys from the repo each reconcile (`sync_tmux_persist`), like the managed config. The web terminal also exposes an **on-demand "Restore sessions" button** (terminal-lobby: `tmux-api` → the validated root `tmux-restore-user` wrapper → `tmux-persist`): the boot-only restore service never fires when an **OOM kills sessions *without* a reboot** (the common case under multi-user memory pressure), so the button covers that gap. It opens a **picker** (`GET /snapshots`, `GET /snapshots/<ts>`, then `POST /restore` with a `{snapshot, sessions[]}` body) showing which snapshot versions exist, how each compares to what is running, and per session what restoring it would do — recreate it, restore alongside as `<name>-<HHMM>` when the name is taken by a different conversation, or resume in place when a session outlived its Claude. Rows resolve server-side so the vanilla lobby and the v2 SPA cannot drift. This is a **tmux/terminal-surface** feature, deliberately outside the t3 namespace: the t3 chat surface persists its own threads (`~/.t3` state, plus the daily `t3-backup-state` dump), and Claude conversations themselves were always durable (`~/.claude/projects/`) — what this adds is the volatile tmux wiring.

**Per-pane memory cap (2026-08-16):** the paragraph above exists because an OOM
can kill sessions *without* a reboot, so the boot-only restore never fires. That
is now much rarer, because each tmux pane runs in its own transient
`tmux-spawn-<uuid>.scope` (tmux 3.4 links libsystemd) and a single top-level
drop-in `/etc/systemd/user/scope.d/50-devvm-pane-cap.conf` caps every one of
them at `MemoryMax=6G` with `OOMPolicy=continue`. At the cap the KERNEL picks
the victim from inside that pane, by raw RSS and with `memory.oom.group=0`, so a
5 GB build a session spawned dies while the ~0.5 GB claude that spawned it keeps
running — and no other user's panes are touched. Before this, the box-wide
`earlyoom` was the only guard that ever fired, and its `--prefer` list contained
`claude`: over the 30 days to 2026-08-16 it made 100 kills, **89 of them
conversations**, at a mean of 600 MiB reclaimed each. `OOMPolicy=continue` is
load-bearing rather than decoration — a scope defaults to `OOMPolicy=stop`, so
systemd tears the whole pane down after a correct single-process kill. The
restore picker is now the fallback for what still gets through (a pane started
before the guard shipped, or a consumer with no pane scope such as a
t3-serve-hosted claude), not the first line of defence. Diagnose an unexplained
`Killed` with
`homelab logs query '{job="devvm-journal"} |= "Memory cgroup out of memory"' --since 24h`.
Design + measurements: `../plans/2026-08-16-devvm-pane-memory-cap.md`; history in
addenda 4-5 of `../post-mortems/2026-06-22-devvm-mem-io-overload-containment.md`.
