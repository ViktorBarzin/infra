...
**The `/model` picker lists exact slugs, from the same file (2026-09-06):** the `modelPicker` key beside `model` replaces Claude Code's four built-in family rows (Default, Sonnet, Opus, Haiku) with one row per slug — `claude-opus-5`, `claude-opus-5[1m]`, `claude-sonnet-5`, `claude-haiku-4-5-20251001`, `claude-opus-4-8`, `claude-fable-5` — because a family word cannot name the thing a person is choosing between: `claude-opus-5`, `claude-opus-5[1m]` and `claude-opus-4-8` are all "opus". The shape is `{ replaceBuiltInOptions, options: [{ model, label, description, behavesAs }] }`, and `behavesAs` is how a slug this build's catalogue does not know is mapped onto one it does. Three things measured on 2026-09-06 against Claude Code 2.1.263, each of which would otherwise cost an afternoon. It is honoured from MANAGED settings only — the same block in a user or project `settings.json` is dropped silently, so this file is the only place to change it. `replaceBuiltInOptions: true` does NOT remove the "Default (recommended)" row, which stays as row 1, so nothing is lost by replacing rather than appending. And a row the account is not entitled to is dropped from the rendered list rather than shown and refused — `claude-fable-5` is declared here and does not currently appear, which is also why `--model claude-fable-5` boots Sonnet 5 with a warning instead of Fable. The rows double as terminal-lobby's model list: its chip on a live session drives this very picker and matches a row by its LABEL (`sessionio/setmodel.go`), so the two lists have to stay in step (`frontend-v2/src/lib/models.ts`).
