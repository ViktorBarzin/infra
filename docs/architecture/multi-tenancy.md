...
**Single source of truth:** `infra/scripts/workstation/roster.yaml` (`os_user → authentik_user / k8s_user / tier / namespaces`). `roster_engine.py` (pytest-covered pure core) derives desired state; `t3-provision-users` (hourly timer) applies it — **additive-only** for existing users (never strips a group, replaces a home, or re-locks an account). `/etc/ttyd-user-map` + `/etc/ttyd-admins` + `dispatch.json` are **generated** from the roster (do not hand-edit). **(2026-08-17: the roster is now the ONLY place the identity map is stored — terminal-lobby carried a second copy at `devvm/ttyd-user-map` which its `deploy.sh` installed over the generated file on every run, and by then it had drifted and would have dropped `ancaelena98=ancamilea`. That copy is deleted; terminal-lobby only reads the generated map. One onboarding step still lives there by decision: the per-user sudo grant in terminal-lobby's `devvm/sudoers.d-ttyd-users` is hand-maintained — the roster says who exists, that file says which binaries may run as them — so a user added to the roster reaches the web terminal's lobby but cannot attach a session until that grant is added and terminal-lobby is deployed.)**
