...
- **ImmichSmartSearchSlow**: Representative context-search ANN query >1s for 15m. The read path has **two** halves in `shared_buffers` and either going cold is enough to trip this. `clip_index` (vchord, ~743MB) holds the quantized codes vchordrq scans; `smart_search`'s **TOAST** relation (~706MB) holds the full-precision vectors it then re-ranks with. Check both gauges, `immich_clip_index_cached_pct` and `immich_smart_search_toast_cached_pct` — a hot `clip_index` beside a cold TOAST is a known shape and was the cause of the nightly 00:07–02:30 slowness through August 2026. Remediation: confirm the `immich-search-probe` CronJob (immich ns, `*/5`) is succeeding, since it runs the prewarm on the :00/:30 ticks. Manual fix, noting that `pg_prewarm` does **not** descend into TOAST, so the table name alone warms only a 13MB stub:
  ```sql
