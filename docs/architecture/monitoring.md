...
- **ContainerNearOOM**: working set above 95% of the container's memory limit for 15m, severity `info` so it routes to `slack-info` and posts once rather than re-pinging. Defined 2026-09-06 at 85%, raised to 95% on 2026-09-19. At 85% five containers were firing continuously and none was near a kill, pg-cluster-2 among them at 85.2%, where a Postgres replica's shared_buffers are working set by design. The trade is a narrower warning window: the cluster's top reading on the day of the change was 93.2%, so the alert is silent at rest and there are five points of a limit between firing and a kill. It is the only pre-mortem memory signal, since `ContainerOOMKilled` and `KernelOOMKiller` both report a kill that already happened.
