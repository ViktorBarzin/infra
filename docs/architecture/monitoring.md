...
**Logs** — `promtail` v3.5.1 (amd64) at `/usr/local/bin/promtail`, config `scripts/pve-promtail.yaml`, unit `scripts/pve-promtail.service`. Ships the journal (path deliberately unset, so it follows `/run/log/journal` now that the journal is volatile, or `/var/log/journal` if that ever changes back) to `https://loki.viktorbarzin.lan/loki/api/v1/push` (`insecure_skip_verify` — the internal `.lan` cert isn't publicly trusted; the name resolves via the Technitium CNAME above, no `/etc/hosts` pin). Relabels: `unit`, `level`, `identifier`; sshd lines (`identifier=~"sshd.*"`) are re-jobbed to `sshd-pve` so the S1 rule matches. Streams:
- `{job="pve-journal", host="pve"}` — full host journal (kernel, pvestatd, fan-control, NFS, etc.).
- `{job="pve-journal", identifier="snoopy"}` — **command audit** (every execve: `uid login tty sid cwd cmdline`). Since 2026-09-24 promtail replaces secret-shaped values with `[REDACTED]` before shipping: bearer, basic and token `Authorization` values, JWTs, `password=`/`secret=`/`token=`/`api_key=` values and `--password`/`--token`/`--secret`/`--api-key` arguments. The local journal (RAM only, root only) keeps the full line. Until then the fan-control loop's Home Assistant token reached Loki on every poll.
...
> Not Terraform. Since 2026-09-24 `playbooks/pve-host.yml` deploys the promtail config (validated with `promtail -check-syntax`, restarted on change) and the fan-control daemon. The promtail binary and unit and the snoopy enable (`/etc/ld.so.preload`) are still set up by hand on the host (Loki resolves via the Technitium CNAME — no `/etc/hosts` pin). Source-of-truth files: `scripts/pve-promtail.{yaml,service}` + `scripts/pve-snoopy.ini`; first-install steps are in the `pve-promtail.yaml` header.
...
- **ContainerNearOOM** (removed 2026-09-23): working set above a share of the container's memory limit for 15m, defined 2026-09-06 at 85% and raised to 95% on 2026-09-19. It was removed to cut alert noise because the containers it reported were almost never the ones that got killed. In its last 7 days it warned for 10 containers, one of which (prometheus-server) was later OOM-killed and reported by `ContainerOOMKilled` anyway, while 3 of that week's 4 kills came with no warning. At 95% it fired 4 times in 4 days, all for the CNPG primary, which peaked at 99.3% of its limit and was not killed. Working set counts active page cache, which the kernel reclaims before a kill, and Postgres fills its cgroup with page cache by design. There is no pre-kill memory signal today: `ContainerOOMKilled` and `KernelOOMKiller` report a kill after it happens. A replacement would need anon memory (`container_memory_rss`), which the cadvisor keep list does not scrape yet.
...
- **ImmichSmartSearchSlow**: Immich's real `searchSmart` query >3s for 15m. The gauge times the query Immich actually issues — asset join, `ownerId`/`visibility`/`deletedAt` filters, and the `ORDER BY embedding <=> $1, asset.id ASC` tiebreaker — against the largest library on the instance. Before 2026-09-06 it timed a bare indexed ANN scan with none of that, which runs in 70-95 ms while the real query takes ~1s, so the gauge under-reported real latency by roughly 15x. Baseline after the change: 834-1088 ms, median 971 ms. Note the tiebreaker makes the vchordrq index unusable for ordering, so Postgres computes the distance for every asset the owner has; that is upstream behaviour, present on `main`, not a local misconfiguration.
- **ImmichSmartSearchIndexSlow**: the bare indexed ANN scan >1s for 15m (normal 0.07-0.095s). This is the old ImmichSmartSearchSlow semantic, now on the series it actually described. It firing means the index or buffer cache is degraded; it staying quiet while ImmichSmartSearchSlow fires means the index is healthy and the cost is the query plan. The read path has **two** halves in `shared_buffers` and either going cold is enough to trip this. `clip_index` (vchord, ~743MB) holds the quantized codes vchordrq scans; `smart_search`'s **TOAST** relation (~706MB) holds the full-precision vectors it then re-ranks with. Check both gauges, `immich_clip_index_cached_pct` and `immich_smart_search_toast_cached_pct` — a hot `clip_index` beside a cold TOAST is a known shape and was the cause of the nightly 00:07–02:30 slowness through August 2026. Remediation: confirm the `immich-search-probe` CronJob (immich ns, `*/5`) is succeeding, since it runs the prewarm on the :00/:30 ticks. Manual fix, noting that `pg_prewarm` does **not** descend into TOAST, so the table name alone warms only a 13MB stub:
  ```sql
  SELECT pg_prewarm('clip_index');
  SELECT pg_prewarm(reltoastrelid) FROM pg_class WHERE relname='smart_search' AND relnamespace='public'::regnamespace;
  ```
- **ImmichClipIndexColdCache**: `clip_index` <50% resident in shared_buffers for 15m (leading indicator; same remediation).
- **ImmichSmartSearchToastColdCache**: the `smart_search` TOAST <50% resident for 15m. The other leading indicator, added 2026-09-06 — until then nothing measured this relation, so the re-rank could run cold with every gauge reading healthy.
...
Immich smart-search monitoring is ONE CronJob in the `immich` namespace since 2026-08-16 — `immich-search-probe` (`*/5`), which absorbed the former `clip-index-prewarm` and `clip-keepalive` jobs (three separate `*/5` jobs firing on the same tick cost 864 pod creations/day for one namespace; merging them changed no cadence and no latency). Its init container re-prewarms on the :00/:30 ticks to keep the vector working set hot during runtime (the `postStart` prewarm only fires at pod start; `pg_prewarm.autoprewarm` only reloads at startup, so it otherwise decays under job buffer-pressure — 48 re-warms a day is ample against a decay measured in days). It warms four relations in a deliberate order — `clip_index`, the `smart_search` TOAST, `smart_search`, the `face_search` TOAST — so that under buffer pressure `face_search` is what loses rather than the smart-search read path. **`pg_prewarm` does not follow a table into its TOAST**, which is why the TOAST relations are addressed explicitly via `reltoastrelid` (the `pg_toast_NNNNN` name is OID-derived and does not survive a dump/restore). Before 2026-09-06 only `clip_index` and the 13MB `smart_search` stub were warmed, leaving ~706MB of re-rank vectors cold, a `warmup` sidecar pings the CLIP textual encoder every tick so the ML model stays resident, and the probe itself (postgres init-container measures a random-vector ANN latency + `pg_buffercache` residency → curl sidecar pushes `immich_smart_search_db_seconds` / `immich_smart_search_probe_wall_seconds` / `immich_clip_index_cached_pct` / `immich_smart_search_toast_cached_pct` / `immich_smart_search_probe_success` / `immich_smart_search_probe_last_run_timestamp` to the Pushgateway). **`immich_smart_search_db_seconds` is what Postgres says the query took** (since 2026-09-04, bead code-9hb8) and is the series `ImmichSmartSearchSlow` reads; it used to carry the wall clock of the `psql` process, so the alert judged psql startup, DNS and connect against a 1 s threshold named after query time. The wall clock is still exported, as `immich_smart_search_probe_wall_seconds`, with no alert on it (`docs/post-mortems/2026-09-03-immich-smart-search-probe-measures-wall-clock.md`). Also surfaced by cluster-health check #46 (`check_immich_search`). Both halves of smart-search warmth — the **Postgres** index and the **ML model** — are now kept warm by this one job.
...
The same trail has a Grafana view since 2026-09-01: dashboard **East-West Traffic** (`dashboards/east-west-traffic.json`, *Networking* folder) over datasource **Goldmane Edges** (`goldmane_edges_datasource.tf`, uid `goldmane-edges-pg`, credential = the aggregator's own Vault-rotated `static-creds/pg-goldmane-edges` mirrored into `monitoring`). It is filterable by namespace / workload / destination port / action and carries an *Unusual connections* panel (first-seen edge, a workload on a port it never used, a namespace's first internet egress). **It depends on the widened `edge` schema** (`migrations/0002_widen_edge.sql` in the aggregator repo) and its panels error until that migration lands; the datasource works either way. Every panel reads `src_workload` / `dst_workload` / `dst_service` and compares endpoint types in **lowercase** (`workload`, `host`, `networkset`, `network`, `unknown` — the aggregator's constants, not Goldmane's `EndpointType` enum spelling), and filters on `NOT pre_widening` so rows predating the widening are left to the `homelab edges` CLI rather than plotted as zeroes. See [runbooks/goldmane-flow-trail.md](../runbooks/goldmane-flow-trail.md) for the full column contract and the limits (no time series, no protocol, burn-in after the migration).
