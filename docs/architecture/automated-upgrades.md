...
- Prometheus halt-on-alert turns ANY firing alert into a hard block — including the 6 Node Runtime Health alerts and the 10 Upgrade Gates alerts that explicitly model "the cluster is in a bad state."
  ...
  **This is a denylist and it does not scale (recorded 2026-09-16, bead `code-rl8j`).** `HALT_IGNORE` now holds five entries: three are the pipeline's own alerts, added to break a self-perpetuating deadlock, one is an ingress-latency alert, and one is `BankSyncConsentExpired`, an expired Amex GoCardless consent that gated 1.35.8 for 2 days 20 hours and can only be cleared by a human re-authorising at the bank. Nothing in the list expresses a general rule, so every future unrelated critical costs another incident to find. The underlying mismatch is that `severity: critical` means "wake a human" while this gate reads it as "the cluster is unfit to upgrade", and those are different sets. The proposed fix is an opt-in `upgrade_gate: "true"` label on the critical rules that genuinely should block. It is a bead rather than a change because **the failure mode inverts**: a missing label fails closed under the denylist and fails **open** under an allowlist, so the chain would drain nodes during a real outage because nobody labelled `KubeAPIDown`. It needs a hardcoded floor of always-blocking alertnames, or a week of shadow-mode logging, before it gates anything. A cheaper partial answer is also open: `halt_on_alert_query` reads Prometheus `/api/v1/alerts` directly and therefore ignores Alertmanager silences entirely, and a silence is arguably an operator saying "I know, proceed".
