...
> **⏸ EXECUTION STATUS (2026-07-19): Fix 1 PAUSED after the Task 1.4 adversarial gate; Fixes 2 & 3 NOT STARTED.**
> **Done:** Task 1.1 (plugin vendored + unit-tested), 1.2 (Middleware CR via `kubectl_manifest` — `kubernetes_manifest` taints on the plugin-shaped spec), 1.3 (kms canary — real-ip attached + verified WAN-200/headerless-200/0-check-failed; kms already had no strip), 1.4 (2 blind challengers → **SAFE-WITH-CONDITIONS**). Plus an unplanned **security hardening**: an `isPublic()` guard on the plugin's CF branch closing a spoofing vector the challengers found (`Cf-Connecting-IP` was client-injectable to any value incl. loopback; now bounded to public IPs; 12/12 tests; deployed + verified spoof-closed live).
> **Held — Viktor's decision (Pause & rethink): Tasks 1.5 (Stage B, fleet default chain) and 1.6 (Stage C, strip removal) are NOT applied.** The gate found fleet-wide is the wrong trade-off — a fleet-SPOF (real-ip would be the *first* Yaegi plugin in the default chain → a plugin-load failure 404s every router) and a wider spoof surface, for a benefit with no current consumer. `drop-x-real-ip` + the proxied-site strip remain the live mechanism on the other 6 Anubis sites. Open redesign questions (trusted-peer check, correct chain position = *after* drop not "first", plugin-load canary, anubis-only vs fleet scope) are in the design doc's **"Post-adversarial-review update"**.
