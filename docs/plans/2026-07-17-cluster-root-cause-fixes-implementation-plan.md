...
> **✅ EXECUTION STATUS (2026-07-19): Fix 1 COMPLETE (scoped anubis-only, not fleet-wide); Fixes 2 & 3 NOT STARTED.**
> **Done:** Tasks 1.1–1.3 (plugin vendored + unit-tested; Middleware CR via `kubectl_manifest` — `kubernetes_manifest` taints on the plugin-shaped spec; kms canary). Task 1.4 gate (2 blind challengers → **SAFE-WITH-CONDITIONS**) surfaced a spoofing hole + a fleet-SPOF. **Resolution (Viktor: pause → then continue safely):** the plugin was rewritten to decide trust by the unspoofable **TCP peer** — it honors `Cf-Connecting-Ip` only from the cloudflared pod peer (`trustedProxyCIDRs = 10.10.0.0/16`), else sets `X-Real-Ip` = the peer. Verified live: raw `X-Real-Ip`, forged `X-Forwarded-For`, and forged `Cf-Connecting-Ip` are all rejected to the real peer; genuine traffic resolves to the true client. real-ip was then attached to **all 7 Anubis sites** via `extra_middlewares`, and the `drop-x-real-ip` Middleware + `strip_x_real_ip` variable/conditional were **deleted**. All 7 verified 200 (edge + WAN), 0 Anubis check-failures, header-less probes 200 (strip-500 hole closed).
> **Superseded (deliberately not done):** Tasks 1.5/1.6 as originally written (fleet-wide default-chain rollout) — replaced by the anubis-only scope, which fully fixes the incident without putting the first Yaegi plugin on every router (the F1 fleet-SPOF).
