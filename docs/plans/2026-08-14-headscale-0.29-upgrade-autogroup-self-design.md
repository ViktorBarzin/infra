...
```stats
0.28.0 → 0.29.3 | headscale version
11 / 11 | nodes retained
113 vs 113 | pfSense capver vs floor
3 | config keys migrated
5 ms | emo laptop ↔ devvm, direct
```
...
| Bare `*` redefined to mean the tailnet ranges, not all IPs | `group:admin`'s `dst: ["*:*"]` stops covering the Sofia LAN, the mgmt/k8s VLANs behind the subnet router, and exit-node internet — no error is raised at apply time |
...
### Two operational notes recorded in `acl.hujson`
...
> read-only operation. Always point it at a copy, never at `/mnt/db.sqlite` or
> at a backup intended for restore.

## Verification

Taken from the live cluster after the apply:
