...
**Status:** DONE — LIVE (2026-07-12). Viktor authorized the bot into the Hermes server; E2E verified in `#hermes`: reply flow, session resume across messages, and an in-pod kubectl query ("6 nodes, all Ready"). Only optional follow-up: wire Executor (`EXECUTOR_MCP_URL` after a UI-minted credential). See §7 As-built.
...
**GO-LIVE (2026-07-12, evening):** Viktor clicked the authorize URL — the bot joined the Hermes guild. Pod restarted; gateway logs `ready: guild=Hermes members=2` (DM member-gate loaded). **E2E verified live in `#hermes`:** (1) reply flow — "Hearing you loud and clear — running on Claude Sonnet."; (2) **session resume** — correctly recalled the prior message's content; (3) **in-pod homelab capability** — kubectl query answered "6 nodes, all Ready." (Bash tool + narrowed RBAC + agentic loop all exercised). Not live-tested (covered by unit tests / needs a second account): rate-limit trip, non-member DM ignore.
