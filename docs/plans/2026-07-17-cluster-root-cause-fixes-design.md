...
- **Status:** Fix 1 **COMPLETE** (2026-07-19) — peer-trust real-ip plugin attached to all 7 Anubis sites (anubis-only scope, not the fleet default chain); the drop-x-real-ip strip machinery is retired. Fixes 2 & 3 not started. See "Post-adversarial-review update" under Fix 1.
- **Date:** 2026-07-17
- **Owner:** Viktor / wizard
...
**Decision & resolution (Viktor, 2026-07-19):** first PAUSED to rethink, then **completed as anubis-only (not fleet-wide)** after closing the spoofing hole at its root. The plugin now decides trust by the **unspoofable TCP peer**: it honors `Cf-Connecting-Ip` only when the peer is a cloudflared pod (`trustedProxyCIDRs = 10.10.0.0/16`) and otherwise sets `X-Real-Ip` = the peer — so no external client can forge it on any path (verified live on kms: raw `X-Real-Ip`, forged `X-Forwarded-For`, and forged `Cf-Connecting-Ip` — even a public one — are all rejected to the real peer; genuine WAN/edge traffic still resolves to the true client). real-ip is attached to **all 7 Anubis sites via `extra_middlewares`** (blog, cyberchef, f1, homepage, jsoncrack, kms, wrongmove), *not* the ingress_factory default chain — which sidesteps the F1 fleet-SPOF (real-ip stays off the ~33 non-Anubis routers). The `drop-x-real-ip` Middleware and the `strip_x_real_ip` variable/conditional are **deleted**. Net: the flap, the strip, the header-less 500, and the spoof surface are all gone, with blast radius confined to the sites that actually read `X-Real-Ip`. All 7 verified 200 (edge + WAN), 0 Anubis check-failures, header-less probes 200.
