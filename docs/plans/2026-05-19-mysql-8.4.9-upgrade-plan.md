...
**Date**: 2026-05-19
**Status**: Drafted, **NOT scheduled**, and now the fallback rather than the
first choice. Read the "2026-09-04 update" in the design document first: the
flush-starvation cause this plan routes around did not reproduce when it was
tested, so a plain image bump is the cheaper thing to price.
**Design**: `2026-05-19-mysql-8.4.9-upgrade-design.md`
**Estimated downtime**: at least 25 min, and the restore alone is now 15 min.
Measured 2026-09-04: restoring all 20 per-db dumps into a fresh 8.4.8 instance
took 918 s, of which nextcloud was 506 s and wrongmove 248 s. The original
25–30 min figure predates the dataset growing to 7,830 MB across 692 tables,
so treat 25 min as the floor and add the wipe, the PVC recreate, the init, the
user recreation and the app restarts on top.
**Target version**: 8.4.11, not the 8.4.9 in the title. 8.4.9 shipped
2026-05-12; 8.4.10 (2026-07-24, an Oracle Critical Patch Update release) and
8.4.11 (2026-07-28) have both superseded it.
...
**Already done, 2026-09-04** (commit `d01934e4`, bead `code-963q`). The
config now carries these values and the running server picked them up:
...
One thing to know about the third line: **the server runs 2 page cleaners, not
4.** MySQL clamps `innodb_page_cleaners` to `innodb_buffer_pool_instances`,
which is 2 here because the buffer pool is 2 GiB and instances are auto-sized
at 1 GiB each. It does this without a warning in the error log. Verified after
the restart:
...
Getting the full 4 means setting `innodb_buffer_pool_instances=4` as well,
which costs another restart of every MySQL tenant. Worth bundling into the next
restart rather than earning one of its own.
