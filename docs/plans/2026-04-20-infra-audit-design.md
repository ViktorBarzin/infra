...
- **Technitium readiness-gate null_resource with `timestamp()` trigger** — Runs every apply, 3-6 min wall time. Replace with a real health-check on `terraform_data` with `triggers_replace = { checksum = sha256(config) }`.
  - *Resolved 2026-09-03 (commit `3e75f574`), partially as proposed.* The `always = timestamp()` trigger is gone, so the gate no longer runs on an apply that changes nothing and the stack plans clean. The four remaining triggers are already `sha256` digests of the three deployment container specs and the Corefile, which is the checksum behaviour this finding asked for, so the resource stayed a `null_resource` rather than migrating to `terraform_data`. Dropping the per-apply run was safe because Prometheus asserts the same properties continuously (`TechnitiumDNSDown`, `TechnitiumZoneCountMismatch`, `TechnitiumZoneSyncStale`, `DNSQueryRateDropped`), which was not established when this finding was written. See `docs/runbooks/technitium-apply.md`.
